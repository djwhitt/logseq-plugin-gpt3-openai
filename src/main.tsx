...
const LogseqApp = () => {
  ...
  const handleCommand = async (
    ...
    if (
      command.completionEngine != null &&
      typeof command.completionEngine === "string"
    ) {
      openAISettings.completionEngine = command.completionEngine;
    }
    const response = await openAIWithStream(
  ...
  const onInsert = async (text: string) => {
    ...
    if (appState.selection.type === "singleBlockSelected") {
      ...
      const lastBlock =
        appState.selection.blocks[appState.selection.blocks.length - 1];
